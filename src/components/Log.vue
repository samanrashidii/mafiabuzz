<template>
    <div class="log-events">
        <template v-if="!log.passiveLog && !log.godLog">
            ...
            " <span class="action-color">{{$t(log.action)}}</span> " on 
            ...
            <i v-if="log.status.invisible && log.ability.playerChecker"> but result is <span :class="{'citizen-role':log.targetMafia}">{{$t('common.Citizen')}}</span> because of " <span :class="{'site-color':true}">{{$t(log.targetPassive)}}</span> "</i>
            <i v-else-if="!log.status.invisible && log.ability.playerChecker"> and result is <span :class="{'mafia-role':log.targetMafia, 'citizen-role':!log.targetMafia}"><span>{{log.targetMafia ? $t('common.Mafia') : $t('common.Citizen')}}</span></span></i>
            <!-- Chef Check Result -->
            <i v-if="log.ability.roleChecker && log.ability.roleChecker"> and result is " <span :class="{'site-color':true}">{{$t(log.targetRole)}}</span> "</i>
            <!-- Cupid Link Result -->
