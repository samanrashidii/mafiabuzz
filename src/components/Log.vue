<template>
    <div class="log-events">
        <template v-if="!log.passiveLog && !log.godLog">
            ...
            <i v-if="log.status.invisible && log.ability.playerChecker"> {{$t('god.logWordBut')}} {{$t('god.logResult')}} <span :class="{'citizen-role':log.targetMafia}">{{$t('common.Citizen')}}</span> {{$t('god.logBecause')}} " <span :class="{'site-color':true}">{{$t(log.targetPassive)}}</span> "</i>
            <i v-else-if="!log.status.invisible && log.ability.playerChecker"> {{$t('god.logWordAnd')}} {{$t('god.logResult')}} <span :class="{'mafia-role':log.targetMafia, 'citizen-role':!log.targetMafia}"><span>{{log.targetMafia ? $t('common.Mafia') : $t('common.Citizen')}}</span></span></i>
            <!-- Chef Check Result -->
        ...
        <template v-else-if="log.godLog">
            <span class="creator-color">{{log.attacker}}</span> {{$t('god.logWordHas')}} 
